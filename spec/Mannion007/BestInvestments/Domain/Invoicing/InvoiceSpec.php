...
class InvoiceSpec extends ObjectBehavior
{
    ...
    public function it_initialises_with_a_consultation_that_is_billable()
    {
        $this->shouldNotThrow(new \DomainException('Consultation is not billable'))->duringInstantiation();
    }

    public function it_does_not_initialise_with_a_consultation_that_is_not_billable(
        Consultation $consultation,
        Money $payAsYouGoRate
    ) {
    ...
    public function it_can_be_paid_when_it_is_outstanding(PaymentReference $paymentReference)
    {
        $this->shouldNotThrow(new \DomainException('Invoice is not outstanding'))->during('pay', [$paymentReference]);
    }

    public function it_cannot_be_paid_when_it_is_not_outstanding(PaymentReference $paymentReference)
    {
        $this->shouldNotThrow(new \DomainException('Invoice is not outstanding'))->during('pay', [$paymentReference]);
